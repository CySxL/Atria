...
- (instancetype)initWithSubtitle:(NSString *)subtitle {
    ...
    if(self) {
        ...
        _description.font = [UIFont systemFontOfSize:14 weight:UIFontWeightMedium];
...
- (void)dismissSelf:(UIButton *)sender {
    // Set the value to YES when the dismiss button is pressed
    [self dismissViewControllerAnimated:YES
                             completion:^{
                                 [[ARITweakManager sharedInstance] setValue:@(YES) forKey:ARIDidSplashPreferenceKey];
                             }];
}
